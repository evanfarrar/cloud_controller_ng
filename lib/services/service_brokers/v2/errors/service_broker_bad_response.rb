      ...
      module Errors
        class ServiceBrokerBadResponse < HttpResponseError
          def initialize(uri, method, response, ignore_description_key: false)
            ...
            if message.bytesize > 2**15
              message = message.truncate_bytes(2**15) + "...This message has been truncated due to size. To read the full message, check the broker's logs"
            end

        ...
        class ServiceBrokerConflict < HttpResponseError
          def initialize(uri, method, response)
            ...
            if parsed_response.is_a?(Hash) && parsed_response.key?('description')
              ...
              if error_message.bytesize > 2**15
                error_message = error_message.truncate_bytes(2**15) + "...This message has been truncated due to size. To read the full message, check the broker's logs"
              end
            end
